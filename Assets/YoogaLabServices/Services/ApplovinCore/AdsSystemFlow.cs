    ...
    public class AdsSystemFlow
    {
        ...
        private void CreateMaxBannerAd()
        {
            ...
            try
            {
                ...
                MaxSdk.CreateBanner(ctx.bannerAdUnitId, adViewConfiguration);
        ...
        public bool IsInterstitialAvailable()
        {
            ...
            if (!remoteConfigContext.ShowInterstitial)
                return false;

            bool isFirstSession =
                sessionManagerContext.IsFirstSession;

            if (isFirstSession &&
                !remoteConfigContext.InterstitialFirstSession)
            {
                return false;
            }
            ...
            int startISDelay =
                remoteConfigContext.StartIS;

            int frequencyCap =
                remoteConfigContext.FrequencyCapIS;

            bool startDelayPassed =
                timeSinceStartup >= startISDelay;

            bool cooldownPassed =
                timeSinceStartup - ctx.lastInterstitialTime >= frequencyCap;

            if (isFirstSession && !startDelayPassed)
                return false;

            if (ctx.hasShownInterstitialThisSession &&
                !cooldownPassed)
            {
                return false;
            }

            return MaxSdk.IsInterstitialReady(ctx.interstitialAdUnitId);
        }
