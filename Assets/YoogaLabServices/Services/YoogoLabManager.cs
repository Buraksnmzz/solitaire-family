...
public class YoogoLabManager
{
    ...
    {
        return YoogaAds.IsInterstitialAvailable();
    }
    ...
    public static void IAP(Product product, Order order, bool isRestore = false)
    {
        if (isRestore)
        {
            Debug.Log("[YoogoLabManager] Skipping IAP tracking because this is a restore call.");
        ...
        {
            ServicesLogger.LogError("[YoogoLabManager] Product or Order is null! Cannot track IAP.");
            return;
        }

        string transactionId = order.Info?.TransactionID;

        Debug.Log($"[YoogoLabManager] Tracking IAP: {transactionId} - {product.definition.id} - " +
                $"{product.metadata.localizedPrice} {product.metadata.isoCurrencyCode}");
